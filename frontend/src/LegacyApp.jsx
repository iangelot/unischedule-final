...
.legacy-app-container {
  ...
  --sw:230px;
  font-family:var(--fb);color:var(--n900);line-height:1.5;font-size:14px;
}
.legacy-app-container button,.legacy-app-container input,.legacy-app-container select,.legacy-app-container textarea{font-family:var(--fb)}
.legacy-app-container button{cursor:pointer;border:none;background:none}
...
import { fetchWithOfflineSupport, getToken, login } from './api';
import { generateTimetable } from './scheduler';

export default function LegacyApp({ embedPage }) {
  const [lang,setLang]=useState("fr");
  const [page,setPage]=useState(embedPage || "dashboard");
  ...
  const [loginEmail, setLoginEmail] = useState("admin@isima.cm");
  ...
  useEffect(() => {
    async function loadData() {
      // Allow loading from local Dexie database even if there's no remote token
      try {
        GROUPS = await fetchWithOfflineSupport('/groups', 'groups') || [];
        LECTURERS = await fetchWithOfflineSupport('/lecturers', 'lecturers') || [];
        ROOMS = await fetchWithOfflineSupport('/rooms', 'rooms') || [];
        COURSES = await fetchWithOfflineSupport('/courses', 'courses') || [];
        const apiSessions = await fetchWithOfflineSupport('/sessions', 'sessions');
        if (apiSessions && apiSessions.length > 0) setSessions(apiSessions);
      } catch (e) {
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    setGenProgress({ phase:"greedy", placed:0, failed:0, score:0, iteration:0, log:['Starting Genetic Algorithm...'] });
    
    // Offload to background (simulated Web Worker via timeout so UI doesn't freeze immediately)
    setTimeout(() => {
      if (cancelRef.current) return;
      try {
        const generatedSessions = generateTimetable(COURSES, LECTURERS, ROOMS, GROUPS);
        // Merge locked sessions with generated ones
        const newPending = [
          ...locked, 
          ...generatedSessions.map(s => ({...s, isGenerated: true}))
        ];
        
        setPendingSessions(newPending);
        setGenProgress({ phase:"done", placed:newPending.length, failed:0, score:100, iteration:100, log:['Generation complete. Schedule is ready!'] });
      } catch (e) {
        console.error(e);
        showToast("Error during generation", "err");
      } finally {
        setGenerating(false);
      }
    }, 200);
  ...
  if (!token && !embedPage) {
    return (
      <div style={{display:'flex',height:'100vh',alignItems:'center',justifyContent:'center',background:'var(--n50)',position:'relative'}}>
        <style>{STYLES}</style>
        
        {/* Language Toggle */}
        <div style={{position:'absolute',top:20,right:20,display:'flex',gap:8}}>
          <button className={`btn btn-sm ${lang==="fr"?"btn-primary":"btn-outline"}`} onClick={()=>setLang("fr")}>FR</button>
          <button className={`btn btn-sm ${lang==="en"?"btn-primary":"btn-outline"}`} onClick={()=>setLang("en")}>EN</button>
        </div>

        <div style={{background:'white',padding:30,borderRadius:'var(--r12)',boxShadow:'0 4px 12px rgba(0,0,0,0.1)',width:320}}>
          <h2 style={{fontFamily:'var(--fd)',marginBottom:6,fontSize:22,color:'var(--g900)'}}>UniSchedule</h2>
          <p style={{fontSize:12,color:'var(--n500)',marginBottom:20}}>{lang==="fr"?"Veuillez vous connecter pour continuer":"Please log in to continue"}</p>
          {loginErr && <div className="alert alert-r" style={{marginBottom:14}}>{IC.alert} {loginErr}</div>}
          <div className="fg">
            <label className="fl">Email</label>
            <input className="fs" value={loginEmail} onChange={e=>setLoginEmail(e.target.value)} placeholder="admin@isima.cm" />
          </div>
          <div className="fg">
            <label className="fl">{lang==="fr"?"Mot de passe":"Password"}</label>
            <div style={{position:'relative'}}>
              <input className="fs" style={{width:'100%',paddingRight:35}} type={showPass?"text":"password"} value={loginPass} onChange={e=>setLoginPass(e.target.value)} placeholder="••••••••" />
              <button 
                onClick={()=>setShowPass(!showPass)} 
                style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',color:'var(--n400)',display:'flex'}}
          ...
          <button className="btn btn-primary" style={{width:'100%',justifyContent:'center',marginTop:10}} onClick={async () => {
             const result = await login(loginEmail, loginPass);
             if (result.success) { setTokenState(getToken()); window.location.reload(); }
             else setLoginErr(result.error);
          }}>{lang==="fr"?"Se connecter":"Log in"}</button>
        </div>
      </div>
    );
  }
  ...
  if (loading) return <div style={{display:'flex',height:'100vh',alignItems:'center',justifyContent:'center',fontFamily:'var(--fb)',color:'var(--n500)'}}>Loading UniSchedule data...</div>;

  // If embedded, only render the specific content + modals/toasts (no sidebar, no topbar layout)
  if (embedPage) {
    return (
      <div className="legacy-app-container" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <style>{STYLES}</style>
        <div className="content" style={{ flex: 1, padding: '24px', overflowY: 'auto' }}>
          {/* We only care about the timetable page for now when embedded */}
