-- ============================================================
-- UniSchedule Africa — Full Database Schema v1.0
-- PostgreSQL 16+   Run: node scripts/migrate.js
-- ============================================================
...
CREATE TABLE IF NOT EXISTS lecturer_blackouts (
  id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  lecturer_id UUID NOT NULL REFERENCES lecturers(id) ON DELETE CASCADE,
  date        DATE NOT NULL,
  start_time  TIME,
...
-- ── INDEXES ────────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_sessions_slot        ON sessions(institution_id,timetable_id,day_of_week,slot_index) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_sessions_lecturer    ON sessions(institution_id,lecturer_id,day_of_week,slot_index)  WHERE deleted_at IS NULL AND status != 'cancelled';
CREATE INDEX IF NOT EXISTS idx_sessions_room        ON sessions(institution_id,room_id,day_of_week,slot_index)      WHERE deleted_at IS NULL AND status != 'cancelled';
CREATE INDEX IF NOT EXISTS idx_session_groups_group ON session_groups(student_group_id,session_id);
CREATE INDEX IF NOT EXISTS idx_timetables_active    ON timetables(institution_id,is_active,status);
CREATE INDEX IF NOT EXISTS idx_conflicts_timetable  ON conflicts(timetable_id,severity,is_resolved);
CREATE INDEX IF NOT EXISTS idx_notif_pending        ON notifications(institution_id,status,channel) WHERE status='pending';
CREATE INDEX IF NOT EXISTS idx_lec_avail            ON lecturer_availability(lecturer_id,day_of_week,is_available);
CREATE INDEX IF NOT EXISTS idx_exams_date           ON exams(institution_id,exam_date) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_audit_entity         ON audit_log(institution_id,entity_type,entity_id);
